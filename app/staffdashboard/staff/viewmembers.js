...
const ViewMembers = ({ userId }) => {
  ...
  const fetchAllMemberDiscounts = async (membersList) => {
  ...
  const fetchMembers = async () => {
    try {
      ...
      // Fetch discounts for all members after loading members
  ...
  const getStatusBadge = (status) => {
    ...
    if (!statusOption) return <Badge variant="outline" className="border-gray-200">Unknown</Badge>

    return (
      <Badge className={`${statusOption.color} border`} variant="outline">
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <DialogFooter className="gap-3 pt-4 border-t">
            ...
            <Button 
              ...
              disabled={isLoading}
              className="bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white shadow-md hover:shadow-lg transition-all duration-200 px-6"
            >
              {isLoading ? (
                <>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
            ...
            <div className="space-y-5 py-4">
              ...
              <div className="bg-green-50 border-l-4 border-green-400 rounded-r-lg p-4">
                <p className="text-sm text-green-900 leading-relaxed">
                  <strong className="font-semibold">Note:</strong> Restoring this account will immediately grant the client full access to the web and mobile application. 
                  They will be able to log in, view their profile, make payments, and use all system features.
                </p>
              </div>
            </div>
          )}
