...
const FreePrograms = ({ userId }) => {
  ...
  const fetchPrograms = async () => {
    ...
    try {
      const url = showArchived ? `${PROGRAMS_API}?staff_id=${userId}&archived=1` : `${PROGRAMS_API}?staff_id=${userId}&archived=0`
    ...
    try {
      ...
      const exercisesData = selectedExercises.map((exerciseId) => {
        ...
        return {
          ...
          reps: details.reps || "",
          sets: details.sets || "",
          repsPerSet: numSets > 1 ? (details.repsPerSet || []) : []
        }
      })
          ...
          <div className="space-y-6 pt-4">
            ...
            <div className="space-y-4">
              ...
              {exercises.length === 0 ? (
                ...
                let filteredExercises = exerciseSearchQuery.trim()
                ...
                filteredExercises = [...selected, ...unselected]
                  ...
                  <>
                    ...
                    {exerciseSearchQuery.trim() && (
                      <div className="text-xs text-muted-foreground">
                        Showing {filteredExercises.length} of {exercises.length} exercises matching "{exerciseSearchQuery}"
                      </div>
                    )}
                  </>
                )
