...
import { Bell, Check, X, Loader2, Search, Info, AlertTriangle, AlertCircle, CheckCircle } from "lucide-react"

const API_URL = "http://localhost/cynergy/notification.php"

const Topbar = ({ searchQuery, setSearchQuery, userRole, userId = 1 }) => {
  ...
  const deleteNotification = async (notificationId) => {
    try {
      ...
      if (response.ok) {
        ...
        if (deletedNotif && deletedNotif.status_name === "Unread") {
          setUnreadCount((prev) => Math.max(0, prev - 1))
        }
      } else {
        toast({
  ...
  return (
    <div className="flex items-center gap-4">
      ...
      <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DropdownMenuContent align="end" className="w-96 p-0 shadow-lg border-0">
          <div className="bg-gradient-to-r from-blue-600 to-purple-600 dark:from-blue-900 dark:to-purple-900 text-white dark:text-gray-100 p-4 rounded-t-lg">
              ...
              <div className="p-2">
                {notifications.map((notification, index) => (
                  <div
                    key={notification.id}
                    className={`
                      ${getNotificationBgColor(notification.type_name, notification.status_name === "Unread")}
                      rounded-lg p-4 mb-2 transition-all duration-200 cursor-pointer group
                    `}
                    onClick={() => {
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className="flex-shrink-0 mt-1">{getNotificationIcon(notification.type_name)}</div>
