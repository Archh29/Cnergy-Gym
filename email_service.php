...
class EmailService {
    ...
    public function sendPasswordResetOTP($userEmail, $userName, $otp) {
        ...
        try {
            $subject = 'CNERGY GYM - Password Reset OTP';
            $htmlBody = $this->createPasswordResetOTPTemplate($userName, $otp);
            $textBody = $this->createPasswordResetOTPPlainText($userName, $otp);
            
            $emailData = $this->createEmailHeaders($userEmail, $userName, $htmlBody, $textBody);
            
            error_log("Attempting to send password reset OTP to: " . $userEmail);
            error_log("Email subject: " . $subject);
            
            $success = mail($userEmail, $subject, $emailData['message'], $emailData['headers']);
            
            if ($success) {
                error_log("Password reset OTP email sent successfully to: " . $userEmail);
                return ['success' => true, 'message' => 'Password reset OTP email sent successfully'];
            } else {
                error_log("Failed to send password reset OTP email to: " . $userEmail);
                return ['success' => false, 'message' => 'Failed to send password reset OTP email - mail() function returned false'];
            }
            
        } catch (Exception $e) {
            error_log("Password reset OTP email sending failed to " . $userEmail . ": " . $e->getMessage());
            return ['success' => false, 'message' => 'Failed to send password reset OTP email: ' . $e->getMessage()];
        }
    }
     ...
     * @return string
     */
    private function createPasswordResetOTPTemplate($userName, $otp) {
        $currentYear = date('Y');
        ...
        <head>
            <meta charset='UTF-8'>
            <meta name='viewport' content='width=device-width, initial-scale=1.0'>
            <title>Password Reset OTP - CNERGY GYM</title>
            <style>
                * {
                    margin: 0;
                    padding: 0;
            ...
            <div class='email-container'>
                ...
                <div class='content'>
                    <div class='otp-section'>
                        <h2 class='otp-title'>Hello, " . htmlspecialchars($userName) . "!</h2>
                        <p class='otp-message'>
                            You have requested to reset your password. Use the OTP code below to verify your identity and reset your password.
                        </p>
                        
                        <div class='otp-code'>
                            <div class='otp-digits'>" . htmlspecialchars($otp) . "</div>
                        </div>
                        
                        <p class='otp-message'>
                            This code will expire in <strong>15 minutes</strong>. If you didn't request this password reset, please ignore this email or contact us immediately.
                        </p>
                        
                        <div class='warning-box'>
                            <p><strong>⚠️ Security Notice:</strong> Never share this OTP code with anyone. CNERGY GYM staff will never ask for your OTP code.</p>
                        </div>
                    </div>
                </div>
    ...
    /**
     * Create plain text version of password reset OTP email
     * @param string $userName
     * @param string $otp
     * @return string
     */
    private function createPasswordResetOTPPlainText($userName, $otp) {
        return "
        CNERGY GYM - PASSWORD RESET OTP
        ================================
        ...
        SECURITY NOTICE: Never share this OTP code with anyone. CNERGY GYM staff will never ask for your OTP code.
